...
public class MyAdapterTratamientosAnteriores extends RecyclerView.Adapter<MyAdapterTratamientosAnteriores.MyViewHolder> {
    private static List<List<String>> tratamientos;
    private List<String> tratamiento;
    private Context contexto;

    MyAdapterTratamientosAnteriores(List<List<String>> myDataset1, List<String> myDataSet2, Context context){
        contexto = context;
    ...
    @NonNull
    @Override
    public MyAdapterTratamientosAnteriores.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.tratamiento_cardview, parent, false);
        return new MyViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull MyAdapterTratamientosAnteriores.MyViewHolder holder, int position) {
        ...
        if(tratamientos.get(position).get(1).equalsIgnoreCase("Crónico")){
            ...
            holder.cardView.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#062240")));
        }
